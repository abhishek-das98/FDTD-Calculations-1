...
task_id = web.upload(sim_mode, task_name = 'cavity_mode')

web.start(task_id)
web.monitor(task_id, verbose = True)

sim_data = web.load(task_id, path='data/sim_data_cavity_mode.hdf5')
print(sim_data.log)

sim_data = td.SimulationData.from_file(fname = 'data/sim_data_cavity_mode.hdf5')

############# data visualization commands are not included here. You can visualize the frequency monitor results directly from the GUI on the Tidy3D website
